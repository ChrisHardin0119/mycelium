...
// You need ~1B spores for first prestige (1 FBE minimum)
export function calculateFBEGain(totalSporesEarned: number): number {
  if (totalSporesEarned < 1e9) return 0;
  return Math.floor(Math.sqrt(totalSporesEarned / 1e9));
}
...
export function performPrestige(state: GameState): GameState {
  ...
  if (fbeGain <= 0) return state; // Can't prestige yet
